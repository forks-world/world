...
workdir=$(mktemp -d)
./bin/world network exec --policy examples/network-offline.json --workdir "$workdir" -- /bin/echo isolated
```

完整控制面、认证、计费、Skill/MCP 和 forkfs RPC 接入仍为拟议规格；本地入口不将 Network 标为已具备完整受管执行能力。
