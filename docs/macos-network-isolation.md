...
- 宿主前缀中的 `..` 只要其后仍有 `tmp` 分量，就由内核解析实际位置后再判断；含 `..` 的相对路径先按物理工作目录或 dirfd 补全再判断。仍是限制的：相对符号链接目标、spawn file actions 中的路径、指向宿主临时目录且其后无 `tmp` 分量的符号链接、以及补全与实际调用之间其他线程 chdir 的竞争。
