...
启动器以参数数组传递命令和内存中的 profile，避免命令字符串拼接及临时策略文件被替换。一个固定的、单线程 shell 启动脚本先关闭标准输入输出之外的描述符，再用 `exec "$@"` 进入 Seatbelt；仅 `/dev/fd` 枚举得到并校验过的数字参与关闭操作，任务参数不插入脚本。这样也能关闭 Go `os/exec` 不会自动关闭的、由宿主传入的非 CLOEXEC socket。清理继承环境，设置私有 HOME/TMPDIR 与 HTTP(S) 代理，清空 NO_PROXY；不继承 DYLD 注入变量、宿主服务凭证，标准输入输出通过管道转发。
